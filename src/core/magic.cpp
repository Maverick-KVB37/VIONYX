...
void init() {
  for (int sq_int = 0; sq_int < 64; ++sq_int) {
    ...
    for (int i = 0; i < bishop_permutations; ++i) {
      Bitboard blockers =
          get_blocker_permutation(i, bishop_bits, bishop_masks[sq_int]);
      Bitboard on_the_fly_attacks =
          generate_bishop_attacks_on_the_fly(sq, blockers);

      Bitboard magic_index =
          (blockers * bishop_magic_numbers[sq_int]) >> (64 - bishop_bits);
    ...
    for (int i = 0; i < rook_permutations; ++i) {
      Bitboard blockers =
          get_blocker_permutation(i, rook_bits, rook_masks[sq_int]);
      Bitboard on_the_fly_attacks =
          generate_rook_attacks_on_the_fly(sq, blockers);

      Bitboard magic_index =
          (blockers * rook_magic_numbers[sq_int]) >> (64 - rook_bits);
