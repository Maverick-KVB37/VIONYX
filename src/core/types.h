...
enum PieceType : I8 { Pawn, Knight, Bishop, Rook, Queen, King, Nonetype };
...
inline constexpr const char *squareToString[65] = {
...
public:
  ...
  S operator+(const S &other) const { return S(mg + other.mg, eg + other.eg); }

  S operator-(const S &other) const { return S(mg - other.mg, eg - other.eg); }

  S operator-() const { return S(-mg, -eg); }

  S operator*(int i) const { return S(mg * i, eg * i); }
};
