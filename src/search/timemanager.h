...
public:
  TimeManager()
      : timeLeft(0), increment(0), movesToGo(0), timeForMove(0),
        stopFlag(false) {}

  // Initialize timer with info from search limits
  void start(const Search::SearchLimits &limits, Color sideToMove,
             int moveNumber = 0);
...
private:
  ...
  int64_t estimateMovesRemaining(int moveNumber, int64_t movesToGo,
